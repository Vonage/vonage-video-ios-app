...
/// Integrates CallKit with an active OpenTok call.
///
...
/// - SeeAlso: ``OpenTokPlugin``, ``OpenTokPluginCallLifeCycle``, ``OpenTokPluginCallHolder``, ``OpenTokCallParams``
/// - Note: The plugin requires a valid `callID` UUID in `userInfo` to drive CallKit.
///   If parsing fails, it throws ``OpenTokCallKitPlugin/Error/invalidCallID``.
public final class OpenTokCallKitPlugin: OpenTokPlugin, OpenTokPluginCallHolder {
    ...
    ///
    /// Defaults to the type name (e.g., `"OpenTokCallKitPlugin"`).
    ...
    /// - ``OpenTokCallParams/roomName``: The display handle for the call
    /// - ``OpenTokCallParams/callID``: A UUID string identifying the CallKit call
    ///
    ...
    /// - Throws: ``OpenTokCallKitPlugin/Error/invalidCallID`` if `callID` cannot be parsed.
    /// - SeeAlso: ``OpenTokCallParams``
