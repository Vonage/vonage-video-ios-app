...
@Suite("AudioDiagnosticsButton Snapshot Tests")
@MainActor
struct AudioDiagnosticsButtonSnapshotTests {

    // MARK: - Test Configuration

    private let isRecording = false  // Set to true to record new snapshots
    private let snapshotPrefix = "AudioDiagnosticsSelectorButton"

    // MARK: - Color Scheme Tests

    @Test(
        "AudioDiagnosticsButton - Color Schemes",
        arguments: [
    ...
    @Test(
        "AudioDiagnosticsButton - Comparison with Camera Selector",
        arguments: [
    ...
    private func makeSUT() -> AudioDiagnosticsButton {
        AudioDiagnosticsButton(makeView: {
            AnyView(
                Text("Mock View")
