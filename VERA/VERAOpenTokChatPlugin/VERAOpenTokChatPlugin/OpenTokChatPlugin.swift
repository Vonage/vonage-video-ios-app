...
/// Enables chat over OpenTok signals and bridges to the chat repository.
///
/// `OpenTokChatPlugin` listens for incoming chat signals, maps them into
...
///
/// - Important: Set `channel` before sending messages; otherwise throws ``OpenTokChatPlugin/Error/missingChannel``.
/// - SeeAlso: ``OpenTokPlugin``, ``OpenTokSignalHandler``, ``OpenTokSignalEmitter``, ``OpenTokSignal``, ``OutgoingSignal``
public final class OpenTokChatPlugin: OpenTokPlugin, OpenTokSignalHandler, OpenTokSignalEmitter {
    ...
    public enum SignalType: String {
        /// Chat messages routed via OpenTok signaling.
        case chat
    }
    ...
    /// Processes an incoming OpenTok signal.
    ///
    /// Accepts only `chat` signals. Attempts to map the signal payload to `ChatMessage`
    /// via ``VERAOpenTok/OpenTokSignal/toChatMessage(date:)`` and stores it in the repository.
    ///
    ...
    ///
    /// - Parameter userInfo: Metadata including ``OpenTokCallParams/username``.
    ...
    ///
    /// Builds an `OpenTokChatMessage`, encodes it to JSON, and emits an ``OutgoingSignal``
    /// with type ``SignalType/chat`` through the configured channel.
    ///
    /// - Parameter message: The text to send.
    /// - Throws: ``OpenTokChatPlugin/Error/missingChannel`` if `channel` is `nil`,
...
/// A message payload used for OpenTok signal transport.
///
...
public struct OpenTokChatMessage: Codable {
    ...
    /// Creates an OpenTok chat payload.
...
extension VERAOpenTok.OpenTokSignal {
    ...
    /// Maps an incoming `OpenTokSignal` to a domain `ChatMessage`.
    ///
